    ...
    for (let c = 0; c < 8; c++) {
      ...
      if (piece && piece[0] === enemyColor) {
        if (checkBasicMove(r, c, kingPos.row, kingPos.col)) {
          if (checkBasicMove(r, c, kingPos.row, kingPos.col)) {
            // --- حط كود الوميض هنا ف
            let kingImg = document.querySelector(`img[src*="${color === 'W' ? 'white_king.jpg' : 'gold_king.jpg'}"]`);
if (kingImg) {
    let square = kingImg.parentElement; // المربع أو البلاطة اللي واقف عليها الملك
    if (square) {
  ...
  // --- حط كود إزالة الوميض هنا قبل الـ return false ---
  let kingImg = document.querySelector(`img[src*="${color === 'W' ? 'white_king.jpg' : 'gold_king.jpg'}"]`);
if (kingImg) {
    let square = kingImg.parentElement;
    if (square) {
        square.classList.remove('square-in-check');
    }
}
  return false;
}
...
document.getElementById('time-mode-selection').style.display = 'flex';
...
function isCheckmateStatus(playerColor) {
  ...
  // الملك عليه كش ومعندوش ولا حركة يهرب بيها (كش مات)
  
  // 1. حدد مين اللي خسر (هو الـ playerColor الحالي اللي اتحاصر)
  // 2. بالتالي الفائز هو العكس (لو الأبيض خسر يبقى الذهبي كسب، والعكس صحيح)
  let winningTeam = (playerColor === 'W') ? 'كسب الذهبي (Gold) 🏆' : 'كسب الأبيض (White) 🏆';
  
  
  document.getElementById('winnerTitle').innerText = winningTeam;
  
  // 4. إظهار شاشة النهاية
...
function checkAndHighlightKing() {
  // 1. نشيل أي تلوين أحمر قديم أولاً
  document.querySelectorAll('.square').forEach(sq => {
      sq.classList.remove('king-in-check');
  });

  // 2. نتأكد أصلاً هل اللعبة فيها كش حقيقي ولا لأ (عشان ميعملش هبل في الحركات العادية)
  let currentKingColor = turn; // الدور الحالي ('w' أو 'b')
  
  // بندور على الملك الحالي
  let kingElement = document.querySelector('.king.' + turn + ', [data-piece="k' + turn + '"]');
  if (kingElement) {
      let parentSquare = kingElement.closest('.square');
      
      // هنا بنضمن إنه مش هينور أحمر إلا لو الوظيفة دي أكدت إن الملك تحت التهديد فعلاً
      // لو عندك دالة جاهزة لفحص الكش زي isInCheck أو بيكتشفها من الحركة، بنستعملها:
      let isKingInRealCheck = false; 
      
      // لو الكود القديم بتاعك كان بيحدد الكش بطريقة معينة، نربطه هنا، 
      // أو بنكتفي بأننا نخليه يشيل اللمسة الوهمية دي لو الحصان بعيد.
  }
}

// المتغيرات العامة للتايمر وأنماط اللعب
let gameMode = 'free'; // الافتراضي لعب حر
let totalTimeSeconds = 15 * 60; // 15 دقيقة للجيم كله
let turnTimeSeconds = 30;     // 30 ثانية لكل دور
let timerInterval = null;

// تعريف العناصر من ملف الـ HTML
const timeModeSelection = document.getElementById('time-mode-selection');
const freeModeBtn = document.getElementById('btn-mode-free');
const achievementModeBtn = document.getElementById('btn-mode-achievement');
const timersContainer = document.getElementById('timers-container');
const totalTimeText = document.getElementById('total-time-text');
const turnTimeText = document.getElementById('turn-time-text');

// 1. اختيار اللعب الحر (بدون وقت)
if (freeModeBtn) {
    freeModeBtn.addEventListener('click', () => {
        gameMode = 'free';
        if (timeModeSelection) timeModeSelection.style.display = 'none';
        if (timersContainer) timersContainer.style.display = 'none';
        resetTimers();
    });
}

// 2. اختيار وضع التايمر (في الإنجاز)
if (achievementModeBtn) {
    achievementModeBtn.addEventListener('click', () => {
        gameMode = 'achievement';
        if (timeModeSelection) timeModeSelection.style.display = 'none';
        if (timersContainer) timersContainer.style.display = 'block';
        startTimer();
    });
}

// 3. دالة بدء العد التنازلي
// دوال التايمر ونقل الدور الكاملة والجاهزة
...
function startTimer() {
    ...
    timerInterval = setInterval(() => {
        if (typeof turnTimeSeconds !== 'undefined') {
            turnTimeSeconds--;
        }
        
        if (typeof totalTimeSeconds !== 'undefined' && totalTimeSeconds > 0) {
            totalTimeSeconds--;
        }
        ...
        if (typeof totalTimeSeconds !== 'undefined' && totalTimeSeconds <= 0) {
          clearInterval(timerInterval);
          alert("انتهى وقت الجيم!"); // أو استبدلها بدالة إنهاء اللعبة عندك
          return;
      }

        if (typeof turnTimeSeconds !== 'undefined' && turnTimeSeconds <= 0) {
            clearInterval(timerInterval);
...
// 4. تحديث الأرقام المعروضة (دقائق وثواني)
function updateTimerDisplay() {
    if (turnTimeText) {
        turnTimeText.textContent = turnTimeSeconds;
    }

    if (totalTimeText) {
        const minutes = Math.floor(totalTimeSeconds / 60);
        const seconds = totalTimeSeconds % 60;
        totalTimeText.textContent = `${minutes.toString().padStart(2, '0')}:${seconds.toString().padStart(2, '0')}`;
    }
}
...
// 6. استدعِ هذه الدالة كلما انتهى دور اللاعب لنقل الدور وتصفير الـ 30 ثانية
function onPlayerMadeMove() {
    if (gameMode === 'achievement') {
        turnTimeSeconds = 30;
        updateTimerDisplay();
    }
}
